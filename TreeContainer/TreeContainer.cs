    ...
    public class TreeContainer : Panel
    {
        ...
        protected override void OnRender(System.Windows.Media.DrawingContext dc)
        {
            ...
            if (Connections != null)
            {
                ...
                SolidColorBrush abrsh = new SolidColorBrush(Colors.Red);
                brsh.Opacity = 0.5;
                abrsh.Opacity = 0.5;
                Pen pen = new Pen(brsh, 1.0);
                Pen apen = new Pen(abrsh, 1.0);
                pen.EndLineCap = PenLineCap.Triangle;
                apen.EndLineCap = PenLineCap.Triangle;
                Point ptLast = new Point(0, 0);
                bool fHaveLastPoint = false;

                foreach (TreeConnection tcn in Connections)
                {
                    Brush chosenb = tcn.Active ? abrsh : brsh;
                    Pen chosen = tcn.Active ? apen : pen;
                    ...
                    foreach (DPoint dpt in tcn.LstPt)
                    {
                        ...
                        dc.DrawLine(chosen, PtFromDPoint(dpt), ptLast);
                        ptLast = PtFromDPoint(dpt);
                    }
                    dc.DrawEllipse(chosenb, chosen, ptLast, 2, 2);
                }
            }
        }
        #endregion
